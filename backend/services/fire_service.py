...
def get_mock_fires() -> list:
    ...
    fires = []
    for i in range(47):
        lat = 30.0 + ((seed_base * (i + 1)) % 1000) / 500.0
        lon = 74.0 + ((seed_base * (i + 5)) % 1000) / 357.0
    ...
    for i in range(23):
        lat = 29.0 + ((seed_base * (i + 50)) % 1000) / 1000.0
        lon = 74.8 + ((seed_base * (i + 55)) % 1000) / 454.0
...
def get_fires_data(db) -> dict:
    ...
    if is_mock:
        ...
        cursor.execute("DELETE FROM fire_cache")
        for f in fires_list:
            cursor.execute('''
                INSERT INTO fire_cache (latitude, longitude, brightness, location_name, timestamp)
                VALUES (?, ?, ?, ?, ?)
            ''', (f["latitude"], f["longitude"], f["brightness"], f["location_name"], datetime.datetime.utcnow()))
        db.commit()
    else:
        try:
            ...
            for row in reader:
                ...
                if 29.0 <= lat <= 32.0 and 73.0 <= lon <= 77.0:
                    ...
                    cursor.execute('''
                        INSERT INTO fire_cache (latitude, longitude, brightness, location_name, timestamp)
                        VALUES (?, ?, ?, ?, ?)
                    ''', (lat, lon, brightness, loc_name, datetime.datetime.utcnow()))
            db.commit()
            
        except Exception as e:
            print(f"Error fetching fires: {e}. Serving from database cache.")
            cursor.execute("SELECT latitude, longitude, brightness, location_name FROM fire_cache")
            cached_fires = cursor.fetchall()
